...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('ordenesempresas', function (Blueprint $table) {
            if (!Schema::hasColumn('ordenesempresas', 'fecha_finalizacion')) {
                $table->dateTime('fecha_finalizacion')->nullable()->after('estado');
            }
            if (!Schema::hasColumn('ordenesempresas', 'fecha_entrega')) {
                $table->dateTime('fecha_entrega')->nullable()->after('fecha_finalizacion');
            }
        });

        // Recrear la vista vista_ordenes para proyectar el nuevo campo fecha_entrega para empresas
    ...
    {
        // Recrear la vista original con NULL para fecha_entrega en empresa
        DB::statement("DROP VIEW IF EXISTS vista_ordenes");
        DB::statement("CREATE VIEW vista_ordenes AS
            select 
                `o`.`id` AS `orden_id`,
        ...
        Schema::table('ordenesempresas', function (Blueprint $table) {
            $table->dropColumn(['fecha_finalizacion', 'fecha_entrega']);
        });
    }
};
