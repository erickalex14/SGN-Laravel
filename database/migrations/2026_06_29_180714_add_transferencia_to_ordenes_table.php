...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('ordenes', function (Blueprint $table) {
            $table->string('transferencia_plataforma', 50)->nullable()->after('fecha_facturacion');
            $table->string('transferencia_numero', 100)->nullable()->after('transferencia_plataforma');
        });
    ...
    public function down(): void
    {
        ...
        Schema::table('ordenes', function (Blueprint $table) {
            $table->dropColumn(['transferencia_plataforma', 'transferencia_numero']);
        });
    }

    private function crearVista(bool $conTransferencia): void
    {
        DB::statement("DROP VIEW IF EXISTS vista_ordenes");

        $colPersonal = $conTransferencia 
            ? "`o`.`transferencia_plataforma` AS `transferencia_plataforma`, `o`.`transferencia_numero` AS `transferencia_numero`"
            : "NULL AS `transferencia_plataforma`, NULL AS `transferencia_numero`";

        $colEmpresa = "NULL AS `transferencia_plataforma`, NULL AS `transferencia_numero`";
