...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('ordenesempresas', function (Blueprint $table) {
            if (!Schema::hasColumn('ordenesempresas', 'estado_repuesto')) {
                $table->string('estado_repuesto', 50)->default('No requerido')->after('estado');
            }
            if (!Schema::hasColumn('ordenesempresas', 'repuesto_inventario_id')) {
                $table->unsignedInteger('repuesto_inventario_id')->nullable()->after('estado_repuesto');
            }
        });

        Schema::table('solicitudesrepuesto', function (Blueprint $table) {
            if (!Schema::hasColumn('solicitudesrepuesto', 'orden_empresa_id')) {
                $table->unsignedInteger('orden_empresa_id')->nullable()->after('orden_id');
            }
        });

        Schema::table('orden_repuestos', function (Blueprint $table) {
            if (!Schema::hasColumn('orden_repuestos', 'orden_empresa_id')) {
                $table->unsignedInteger('orden_empresa_id')->nullable()->after('orden_id');
            }
        });

        // Hacer orden_id nullable en solicitudesrepuesto y orden_repuestos
        DB::statement("ALTER TABLE solicitudesrepuesto MODIFY orden_id INT NULL;");
        DB::statement("ALTER TABLE orden_repuestos MODIFY orden_id INT NULL;");

        // Recrear la vista vista_ordenes para proyectar el nuevo campo de repuesto para empresas
        ...
        DB::statement("CREATE VIEW vista_ordenes AS
            ...
            select 
                ...
                (coalesce(`oe`.`estado_repuesto`, 'No requerido') collate utf8mb4_0900_ai_ci) AS `estado_repuesto`,
                ...
                date_format(`oe`.`fecha_entrega`,'%d/%m/%Y') AS `fecha_entrega_fmt` 
    ...
    public function down(): void
    {
        ...
        DB::statement("ALTER TABLE solicitudesrepuesto MODIFY orden_id INT NOT NULL;");
        DB::statement("ALTER TABLE orden_repuestos MODIFY orden_id INT NOT NULL;");

        Schema::table('ordenesempresas', function (Blueprint $table) {
            $table->dropColumn(['estado_repuesto', 'repuesto_inventario_id']);
        });

        Schema::table('solicitudesrepuesto', function (Blueprint $table) {
            $table->dropColumn('orden_empresa_id');
        });
