...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('ordenesempresas', function (Blueprint $table) {
            $table->string('subtipo', 50)->change();
            if (!Schema::hasColumn('ordenesempresas', 'nro_sucursal_cliente')) {
                $table->char('nro_sucursal_cliente', 5)->nullable()->after('subtipo');
            }
        });

        // Recrear la vista vista_ordenes para proyectar el nuevo campo para empresas
    ...
    {
        // Recrear la vista original con NULL para nro_sucursal_cliente en empresa
        DB::statement("DROP VIEW IF EXISTS vista_ordenes");
        DB::statement("CREATE VIEW vista_ordenes AS
            select 
                `o`.`id` AS `orden_id`,
        ...
        Schema::table('ordenesempresas', function (Blueprint $table) {
            $table->dropColumn('nro_sucursal_cliente');
            $table->enum('subtipo', ['Autoconsumo', 'Servicios'])->change();
        });
    }
};
